...
const DEFAULT_DATA = {
  categories: ["Promotores de conciertos", "Festivales de música", "Artistas", "Giras (participamos)", "Agencias de MGMT"],
  clients: [
    ...
    { id: 30, name: "Judeline", category: "Artistas" },
    { id: 31, name: "Sanguijuelas del Guadiana", category: "Artistas" },
    { id: 32, name: "Kiddo", category: "Artistas" },
    { id: 33, name: "La Raíz", category: "Artistas" },
    { id: 34, name: "Barry B", category: "Artistas" },
    { id: 35, name: "Hard GZ", category: "Artistas" },
    { id: 36, name: "Víctor Martínez", category: "Artistas" },
    { id: 37, name: "Sexy Zebras", category: "Artistas" },
    { id: 38, name: "La M.O.D.A.", category: "Artistas" },
    { id: 39, name: "Zetak Mitoaroa I, II, III", category: "Artistas" },
    { id: 40, name: "Amparanoia", category: "Artistas" },
    { id: 50, name: "Katy Perry", category: "Giras (participamos)" },
    { id: 51, name: "Shawn Mendes", category: "Giras (participamos)" },
    { id: 52, name: "Yerai Cortés", category: "Giras (participamos)" },
    { id: 53, name: "María Becerra", category: "Giras (participamos)" },
    { id: 54, name: "Carolina Durante", category: "Giras (participamos)" },
    { id: 55, name: "Rosario Flores", category: "Giras (participamos)" },
    { id: 56, name: "Amaral", category: "Giras (participamos)" },
    { id: 57, name: "Bustamante", category: "Giras (participamos)" },
    { id: 58, name: "Antonio Orozco", category: "Giras (participamos)" },
    { id: 59, name: "Alba Reche", category: "Giras (participamos)" },
    { id: 60, name: "Pablo López", category: "Giras (participamos)" },
    { id: 61, name: "Morad", category: "Giras (participamos)" },
    { id: 62, name: "La Zowi", category: "Giras (participamos)" },
    { id: 70, name: "Polar", category: "Agencias de MGMT" },
  ],
};
...
function getStoredData() {
  ...
  try {
    const raw = localStorage.getItem("sftl-clients-v3");
    return raw ? JSON.parse(raw) : DEFAULT_DATA;
  } catch { return DEFAULT_DATA; }
}

function storeData(d) {
  try { localStorage.setItem("sftl-clients-v3", JSON.stringify(d)); } catch {}
}
...
export default function Home() {
  ...
  const addClient = () => {
    ...
    const c = { id: Date.now(), name: newName.trim(), category: newCat, promotor: newCat === "Festivales de música" && newPromotor ? newPromotor : null };
    save({ ...data, clients: [...data.clients, c] });
    setNewName(""); setNewCat(""); setNewPromotor(""); setShowAddClient(false);
  };
  ...
  const saveEdit = (id) => {
    ...
    let updated = data.clients.map(c => {
      if (c.id === id) return { ...c, name: editName.trim(), category: editCatName || c.category, promotor: editCatName === "Festivales de música" ? (editPromotor || null) : c.promotor };
      return c;
    });
  ...
  const getFestivalesByPromotor = (name) => data.clients.filter(c => c.category === "Festivales de música" && c.promotor === name);
        ...
        <div className="max-w-2xl mx-auto px-4 py-5">
          ...
          <div className="flex items-center justify-between mb-4 mt-3">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setShowAddClient(true); setNewCat(cats[0] || ""); setNewPromotor(""); }} style={{ backgroundColor: "#F2E2A4", color: "#1E1E1C" }} className="px-3 py-2 text-xs rounded-xl font-semibold hover:opacity-90 transition">+ Cliente</button>
            </div>
          </div>
            ...
            <div key={cat}>
              ...
              {clients.length === 0 ? (
                ...
                <div className="space-y-2">
                  {clients.map(c => {
                    const isPromotor = c.category === "Promotores de conciertos";
                    const linked = isPromotor ? getFestivalesByPromotor(c.name) : [];
                    const isExp = expandedPromotor === c.id;
                    const hasLinked = linked.length > 0;
                    return (
                      <div key={c.id}>
                        <div style={{ backgroundColor: "#252523", borderColor: "#3a3a38" }} className={`rounded-2xl border p-3.5 flex items-center justify-between hover:border-yellow-800 transition group ${isPromotor && hasLinked ? "cursor-pointer" : ""}`} onClick={() => isPromotor && hasLinked && setExpandedPromotor(isExp ? null : c.id)}>
                          {editingId === c.id ? (
                            <div className="flex-1 flex gap-2 items-center flex-wrap" onClick={e => e.stopPropagation()}>
                              ...
                              {editCatName === "Festivales de música" && (
                            ...
                            <>
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                  ...
                                  {isPromotor && hasLinked && <p style={{ color: "#F2E2A444" }} className="text-xs mt-0.5">{linked.length} festival{linked.length !== 1 ? "es" : ""} vinculado{linked.length !== 1 ? "s" : ""}</p>}
                                </div>
                              </div>
                              <div className="flex items-center gap-1">
                                {isPromotor && hasLinked && <span style={{ color: "#F2E2A444" }} className="text-xs mr-1">{isExp ? "▲" : "▼"}</span>}
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition" onClick={e => e.stopPropagation()}>
                                  <button onClick={() => { setEditingId(c.id); setEditName(c.name); setEditCatName(c.category); setEditPromotor(c.promotor || ""); }} style={{ color: "#F2E2A466" }} className="p-1.5 rounded-lg text-xs hover:opacity-100">editar</button>
                        ...
                        {isPromotor && isExp && hasLinked && (
                          <div style={{ borderColor: "#73FBF944" }} className="ml-6 mt-1 space-y-1 border-l-2 pl-3">
                            {linked.map(f => (
                              <div key={f.id} style={{ backgroundColor: "#73FBF910" }} className="rounded-xl px-3 py-2 flex items-center gap-2">
      ...
      {showAddClient && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4" onClick={() => setShowAddClient(false)}>
          <div style={{ backgroundColor: "#252523", borderColor: "#3a3a38" }} className="rounded-2xl border p-6 w-full max-w-sm shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <select value={newCat} onChange={e => { setNewCat(e.target.value); setNewPromotor(""); }} style={{ backgroundColor: "#2a2a28", borderColor: "#3a3a38", color: "#F2E2A4" }} className="w-full px-4 py-2.5 rounded-xl border text-sm mb-3 focus:outline-none">
              {cats.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            {newCat === "Festivales de música" && promotores.length > 0 && (
